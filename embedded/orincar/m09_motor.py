...
class motor_control:
    ...
    # 버튼 클릭으로만 조타하는 용도
    def btn_left(self):
        self.steer_value_macro = max(-1, self.steer_value_macro - 1)
        self.set_steering(self.steer_value_macro * 0.75)

    def btn_right(self):
        self.steer_value_macro = min(1, self.steer_value_macro + 1)
        self.set_steering(self.steer_value_macro * 0.75)

    # ssh등으로 하는 용도
    def left(self):
        self.steer_value_macro = -1
